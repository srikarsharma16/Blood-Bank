...
class DBConnect {
    ...
    public function searchDonorByCity($city){
        $stmt = $this->db->prepare("SELECT * FROM users WHERE city LIKE ?");
        $stmt->execute(["%".$city."%"]);
        return $stmt->fetchAll();
    }
    
    public function searchDonorWithBloodGroupCity($bloodGroup,$city){
        $stmt = $this->db->prepare("SELECT * FROM users WHERE b_type LIKE ? and city like ?");
